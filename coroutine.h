...
inline std::ostream& operator <<(std::ostream& os, const ResumeRet& ret)
{
    if( ret == INVALID)          os << "INVALID";
    else if( ret == FINISHED)    os << "FINISHED";
    else if( ret == YIELD)  os << "yield";

    return os;
}

inline ResumeRet resume(routine_t id)
{
    assert(ordinator.current == 0);
    ...
    return routine->finished ? FINISHED : YIELD;
}
